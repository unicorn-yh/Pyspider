...
class Handler(BaseHandler):
       ...
       ''' for each in response.doc('a[href^="http"]').items():
        ...
        for tag in Tag('div[class="e"]').items():
            try:
                ...
                #daterelease
                #zhiweiyaoqiu
                self.lianjie=url=tag.find("el").attr.href
            except Exception as e:
                pass
    ...
    def detail_page(self, response):
        try:
            Tag=response.doc('div[class="bmsg job_msg inbox"]')
            subTags=Tag('div[class="e"]').items()
            ls=[]
            key=0
            for tag in subTags:
                if "任职要求" in tag('p').text():
                    key=1
                if "任职要求" not in tag('p').text() and key==0:
                    continue;
                else:
                    ls.append(tag('p').text().strip())
            self.zhiweiyaoqiu=ls
        except:
            self.zhiweiyaoqiu="无"
        ...
        with open('"C:\\Users\\User\\Desktop\\51data.csv','a',encoding='utf-8',newline='') as csvfile:
            ...
            writer.writeheader()
            tempdict={"职位":self.zhiwei,"公司":self.gongsi,"地点":self.didian,"薪资":self.xinzi,"公司类型":self.gongsileixing,"规模":self.guimo,"行业":self.hangye,"经验":self.jingyan,"学历":self.xueli,"招聘人数":self.renshu,"福利":self.fuli,"职位要求":self.zhiweiyaoqiu,"链接":self.lianjie}
            writer.writerow(tempdict)
            #writer.writerow(self.head)
            #writer.writerow(self.tempdata)
        print("succeessfull!")
